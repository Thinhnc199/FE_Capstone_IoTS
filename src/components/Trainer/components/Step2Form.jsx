...
//       return;
//     }
//     const videoData = { ...values, labId, videoFile: file };
...
//         videoUrl: video.videoUrl?.url || "", // Xử lý videoUrl là object
//       });
...
//               title: video.title,
//               description: video.description,
//               videoUrl: result.id, // API trả về id, không phải object
//             });
...
//             description: video.description,
//             videoUrl:
//               typeof video.videoUrl === "object"
//                 ? video.videoUrl.id
//                 : video.videoUrl, // Xử lý videoUrl là object
//           });
//         }
//       }
...
//     // Xử lý videoUrl để đảm bảo là chuỗi
//     const videoUrl =
//       typeof video.videoUrl === "object" ? video.videoUrl.url : video.videoUrl;
...
//             icon={<EyeOutlined />}
//             onClick={() => {
//               setCurrentVideoUrl(videoUrl || ""); // Đảm bảo videoUrl là chuỗi
...
const Step2Form = ({ labId, onBack }) => {
  ...
  const handleFileChange = async ({ fileList }) => {
    ...
    if (selectedFile) {
      ...
      if (isValid) {
        ...
        setFile(null);
        form.setFieldsValue({ videoUrl: form.getFieldValue("videoUrl") }); // Giữ nguyên videoUrl cũ
      }
    } else {
      setFile(null);
    }
  ...
  const handleInsert = (values) => {
    ...
    const videoData = {
      ...values,
      labId,
      videoFile: file, // Có thể là null nếu không chọn video mới
      videoUrl: file ? undefined : values.videoUrl, // Giữ videoUrl cũ nếu không chọn file mới
    };
    setVideos([videoData, ...videos]);
    form.resetFields();
    setFile(null);
        ...
        if (video.videoFile) {
          try {
            ...
            playlistData.push({
              ...
              title: video.title,
              description: video.description,
              videoUrl: result.id, // API trả về id
            });
            setUploadProgress(((i + 1) / videos.length) * 100);
          } catch (err) {
            notification.error({
          ...
          playlistData.push({
            ...
            title: video.title,
            description: video.description,
            videoUrl: video.videoUrl, // Giữ videoUrl cũ nếu không có videoFile
          });
        }
      }
  ...
  SortableItem.propTypes = {
    video: PropTypes.shape({
      title: PropTypes.string,
      description: PropTypes.string,
      videoUrl: PropTypes.string, // videoUrl là chuỗi
    }).isRequired,
    index: PropTypes.number.isRequired,
  };
